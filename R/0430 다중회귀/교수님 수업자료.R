...
s <- sd(weight)            # 여자 신생아 몸무게의 표준편차(standerd deviation)
n <- length(weight)        # weight의 행 개수
h0 <- 2800                 # 모집단의 평균(기존의 여아의 몸무게 평균 = 2800)

t.t <- (barx - h0) / (s / sqrt(n))   # t 검정 통계량

alpha <- 0.05
c.u <- qt(1-alpha, df=n-1)            # 정규분포 범위가 0.95가 되는 위치. (누적 확률이 0.95가 되는 위치.)
p.value <- 1 - pt(t.t, df=n-1)        # 검정통계량 2.23 이상이 될 확률.



# R에서 제공하는 t-test
t.test(weight, mu=2800, alternative="greater")  # mu 는 모집단의 평균, alternative는 뭘 말하는 거라고...????????
                                                # 실행하면 검정값(신뢰구간, p-value, 등등)에 채택된 대안가설까지 보여준다.
...
plot(x, y, type="l", axes=F, ylim=c(-0.02, 0.38), 
     main="", xlab="t", ylab="")
abline(h=0)     # abline h(horizental, 수평.) = 0 즉, x 절편을 그리라는 이야기.
...
hist(data$weight[data$gender==2], density=10, angle=20, 
     add=TRUE, col="green")
axis(1)
...
# 퍼져 있는 정도를 통해 분산이 같은지 안 같은지 알 수 있다. (그래프 모양 보고.)

# 등분산성 테스트
var.test(data$weight ~ data$gender)           # p > 0.05, 분산이 같다.(귀무가설, 즉 영가설 채택.)
var.test(weight ~ gender, data=data)

# 2-sample T test
t.test(data$weight ~ data$gender, mu=0, alternative="less", 
       var.equal=TRUE )                              # 대안 가설이 틀렸으니 영가설(귀무가설)을 채택.

# 3. paired T-test                       # 사전/사후 비교.
# 식욕부진증 치료요법의 효과 검정
